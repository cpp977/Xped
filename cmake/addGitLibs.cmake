...
if(${XPED_TENSOR_LIB} STREQUAL "Cyclops" AND XPED_BUILD_CYCLOPS)
  ...
  if(XPED_USE_MKL)
    get_target_property(XPED_USED_MKL_LINKER_FLAGS MKL::MKL INTERFACE_LINK_LIBRARIES)
    set(XPED_USED_MKL_LIBS "")
    foreach(MKL_LIB ${XPED_USED_MKL_LINKER_FLAGS})
      message(STATUS "Lib: ${MKL_LIB}")
      string(FIND ${MKL_LIB} "MKL::" HAS_MKL_IN_NAME)
      string(FIND ${MKL_LIB} "-" HAS_DASH_IN_NAME)
      if(NOT(${HAS_MKL_IN_NAME} EQUAL -1) AND (${HAS_DASH_IN_NAME} EQUAL -1))
        string(REPLACE "MKL::" "-l" LINK_ENTRY ${MKL_LIB})
        message(STATUS "Link entry "${LINK_ENTRY})
        list(APPEND XPED_USED_MKL_LIBS " ${LINK_ENTRY}")
      elseif(NOT(${HAS_MKL_IN_NAME} EQUAL -1) AND NOT(${HAS_DASH_IN_NAME} EQUAL -1))
        
      elseif(NOT(${HAS_DASH_IN_NAME} EQUAL -1))
        message(STATUS "Link entry "${MKL_LIB})
        list(APPEND XPED_USED_MKL_LIBS " ${MKL_LIB}")
      endif()
    endforeach()
  #   if(XPED_MKL_USE_MPI)
  #     get_target_property(XPED_USED_MKL_SCALAPACK_LINKER_FLAGS MKL::MKL INTERFACE_LINK_LIBRARIES)
  #     foreach(MKL_LIB ${XPED_USED_MKL_SCALAPACK_LINKER_FLAGS})
  #       string(FIND ${MKL_LIB} "MKL::" IS_CMAKE_TARGET)
  #       if(NOT(${IS_CMAKE_TARGET} EQUAL -1))
  ...
  #           string(FIND ${MKL_LIB_REC} "MKL::" IS_CMAKE_TARGET_REC)
  #           if(NOT(${IS_CMAKE_TARGET_REC} EQUAL -1))
  ...
  #   # endif()
   list(APPEND cmd_configure " LIBS=\" -L${MKLROOT}/lib/intel64 -lmkl_scalapack_lp64 -Wl,--no-as-needed -lmkl_intel_lp64 -lmkl_gnu_thread -lmkl_core -lmkl_blacs_intelmpi_lp64 -lgomp -lpthread -lm -ldl\"")
   list(APPEND cmd_configure " LD_LIBS=\" -L${MKLROOT}/lib/intel64 -lmkl_scalapack_lp64 -Wl,--no-as-needed -lmkl_intel_lp64 -lmkl_gnu_thread -lmkl_core -lmkl_blacs_intelmpi_lp64 -lgomp -lpthread -lm -ldl\"")
  endif()
