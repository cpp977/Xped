#ifndef XPED_REVERSE_PASS_CALLBACK_ALLOC_HPP_
#define XPED_REVERSE_PASS_CALLBACK_ALLOC_HPP_
...
template <typename F>
struct reverse_pass_callback_alloc_vari : public stan::math::vari_base, public stan::math::chainable_alloc
{
    F rev_functor_;

    explicit reverse_pass_callback_alloc_vari(F&& rev_functor)
 ...
 */
template <typename F>
inline void reverse_pass_callback_alloc(F&& functor)
{
    new internal::reverse_pass_callback_alloc_vari<F>(std::forward<F>(functor));
}
