...
public:
        ...
        : Hamiltonian<Scalar, Symmetry>(params_in, pat_in, bond_in, "Heisenberg", sym)
    {
        ...
        if constexpr(std::is_same_v<Symmetry, Sym::SU2<Sym::SpinSU2>>) {
            ...
            gate_nn = this->params["Jz"].template get<double>() * tprod(B.Sz(), B.Sz(0, C4v_sym)) +
                      0.5 * this->params["Jxy"].template get<double>() * (tprod(B.Sp(), B.Sm(0, C4v_sym)) + tprod(B.Sm(), B.Sp(0, C4v_sym)));
        } else {
    ...
    virtual void setDefaultObs() override
    {
        ...
        if constexpr(std::is_same_v<Symmetry, Sym::SU2<Sym::SpinSU2>>) {
            if(C4v_sym) {
                auto SS = std::make_unique<Xped::TwoSiteObservable<double, Symmetry>>(this->pat, Xped::Opts::Bond::H | Xped::Opts::Bond::D1, "SS");
                for(auto& t : SS->data_h) { t = std::sqrt(3.) * Xped::tprod(B.Sdag(), B.S(0, C4v_sym)); }
            ...
            if(C4v_sym) {
                auto SzSz =
                    std::make_unique<Xped::TwoSiteObservable<double, Symmetry>>(this->pat, Xped::Opts::Bond::H | Xped::Opts::Bond::D1, "SzSz");
                for(auto& t : SzSz->data_h) { t = Xped::tprod(B.Sz(), B.Sz(0, C4v_sym)); }
            ...
            if constexpr(not Symmetry::ANY_IS_SPIN) {
                if(C4v_sym) {
                    auto SxSx = std::make_unique<TwoSiteObservable<double, Symmetry>>(this->pat, Opts::Bond::H | Opts::Bond::D1, "SxSx");
                    for(auto& t : SxSx->data_h) { t = tprod(B.Sx(), B.Sx(0, C4v_sym)); }
