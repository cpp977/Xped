...
//forward declaration
template<typename Symmetry, std::size_t depth> class Qbasis;
...
class Basis
{
        template<typename Symmetry_, std::size_t depth> friend class Qbasis;
public:
	/**Does nothing.*/
	Basis() {};
	
	Basis(std::size_t dim_in): dim_(dim_in) {};
