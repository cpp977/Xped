...
struct tensortraits<EigenTensorLib>
{
    ...
    template <typename Scalar, std::size_t Rank>
    using Maptype = Eigen::TensorMap<Ttype<Scalar, Rank>>;
    template <typename Scalar, std::size_t Rank>
    using cMaptype = Eigen::TensorMap<cTtype<Scalar, Rank>>;
    ...
    // template<typename Scalar, std::size_t Rank, Indextype... p>
    // static auto shuffle_view(const cMaptype<Scalar,Rank>& T)
    // {
    //         static_assert(Rank == sizeof...(p));
    //         std::array<Indextype, Rank> dims = {p...};
    //         return T.shuffle(dims);
...
struct tensortraits<CyclopsTensorLib>
{
    ...
    template <typename Scalar, std::size_t Rank>
    using Maptype = CTF::Tensor<Scalar>;
    template <typename Scalar, std::size_t Rank>
    using cMaptype = const CTF::Tensor<Scalar>;
    ...
    static cMaptype<Scalar, Rank> cMap(const Scalar* data, const std::array<Indextype, Rank>& dims)
    {
        Maptype<Scalar, Rank> out(Rank, dims.data());
        int64_t nvals;
        int64_t* indices;
        Scalar* new_data;
    ...
    static Ttype<Scalar, Rank> tensorProd(Ttype<Scalar, Rank>& T1, Ttype<Scalar, Rank>& T2)
    {
        ...
        Ttype<Scalar, 2 * Rank> tmp(2 * Rank, tmp_dims.data());
        ...
        Ttype<Scalar, Rank> out(Rank, out_dims.data());
        int64_t nvals;
        int64_t* indices;
        Scalar* data;
                                ...
                                seq::iseq<Indextype, Ist...> St)
    {
        ...
        Ttype<Scalar, sizeof...(Ist)> res(sizeof...(Ist), lens);
    ...
    static Ttype<Scalar, Rank2> reshape(const Ttype<Scalar, Rank1>& T, const std::array<Indextype, Rank2>& dims)
    {
        Ttype<Scalar, Rank2> out(Rank2, dims.data());
        int64_t nvals;
        int64_t* indices;
        Scalar* new_data;
