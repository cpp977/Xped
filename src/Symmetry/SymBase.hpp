...
struct SymBase
{
    ...
    static std::vector<qType> reduceSilent(const qType& ql, const qType& qr) { return Derived::basis_combine(ql, qr); }

    static std::vector<qType> reduceSilent(const std::vector<qType>& ql, const qType& qr)
    {
        std::vector<qType> vout;
        for(const auto& q : ql) {
            for(const auto& qmerge : Derived::basis_combine(q, qr)) { vout.push_back(qmerge); }
        }
        return vout;
    }
    ...
    static std::set<qType> reduceSilent(const std::vector<qType>& ql, const std::vector<qType>& qr)
    {
        ...
        for(const auto& q : ql) {
            for(const auto& p : qr) {
                for(const auto& qmerge : Derived::basis_combine(q, p)) { vout.insert(qmerge); }
            }
        }
        return vout;
    }
    ...
    static std::set<qType> reduceSilent(const std::set<qType>& ql, const std::vector<qType>& qr)
    {
        ...
        for(const auto q : ql) {
            for(const auto& p : qr) {
                for(const auto qp : Derived::basis_combine(q, p)) { out.insert(qp); }
            }
        }
        return out;
    }
    ...
    {
        return std::signbit(Derived::coeff_recouple(q, Derived::conj(q), q, q, Derived::qvacuum(), Derived::qvacuum())) ? Scalar(-1) : Scalar(1.);
    }

    static Scalar coeff_turn(const qType& ql, const qType& qr, const qType& qf)
    {
        return std::sqrt(Derived::degeneracy(qr)) * Derived::coeff_recouple(ql, qr, Derived::conj(qr), ql, qf, Derived::qvacuum());
    }
    ...
    static bool compare(const qType& q1, const qType& q2) { return q1 < q2; }
