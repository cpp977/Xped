...
Sz(const std::size_t& site) const
{
	std::vector<Eigen::Triplet<Scalar> > tripletList;
	for (size_t i=0; i<basis_.dim(); i++)
	{		
		tripletList.push_back(Eigen::Triplet<Scalar>(i,i,basis_.sz_vector(i)[site]));
	}
...
MatrixType Heisenberg::
Sp(const std::size_t& site) const
{
	std::vector<Eigen::Triplet<Scalar> > tripletList;
	for (size_t i=0; i<basis_.dim(); i++)
	{
...
MatrixType Heisenberg::
H(const Eigen::Matrix<Scalar,-1,-1>& J) const
{
	assert(J.rows() == L() and J.cols() == L());
	MatrixType out(basis_.dim(),basis_.dim()); out.setZero();
