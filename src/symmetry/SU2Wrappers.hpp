...
/** @file
 * In this file, the wrappers are defined to include different external libraries which compute \f$3nj\f$-symbols for \f$SU(2)\f$. The wrappers are
 * called coupl_Xj_base. All of these functions take values in the format q=2S+1 and q_z=2M which is always of type integer. Internally these values
 * are converted to the respective convention of the used library. For example gsl uses the convention q_gsl=2S and q_z_gsl=2M, so that we have to use
 * q_gsl = q-1 and q_gsl_z = q_z. Currently, one can use three different libraries for the \f$3nj\f$-symbols:
 *
 ...
 * \note The hash-tables need to be precomputed by the user and passed to Sym::initialize(). See manual http://fy.chalmers.se/subatom/fastwigxj/README
 * for instructions. The precomputed tables are commonly named as table_Y.Xj, where X is {3,6,9} and Y the maximal value of J. The tables are commonly
 * stored in the folder cgc_hash within the root-folder of the project. \note For 9j-symbols which are not pre-computed, the library uses a fallback
 * and compute the 9j-symbol from 6j-symbol. To do so, the library needs 128bit floats (quadmath). This enforces the compiler gcc, since clang does
 * not support quadmath. \note When extensively using this library you may cite the corresponding publication (See website above for details)
...
{
    return gsl_sf_coupling_9j(q1 - 1, q2 - 1, q3 - 1, q4 - 1, q5 - 1, q6 - 1, q7 - 1, q8 - 1, q9 - 1);
}

inline double coupl_6j_base(const int q1, const int q2, const int q3, const int q4, const int q5, const int q6)
{
    return gsl_sf_coupling_6j(q1 - 1, q2 - 1, q3 - 1, q4 - 1, q5 - 1, q6 - 1);
}
...
coupl_9j_base(const int q1, const int q2, const int q3, const int q4, const int q5, const int q6, const int q7, const int q8, const int q9)
{
    return wig9jj(q1 - 1, q2 - 1, q3 - 1, q4 - 1, q5 - 1, q6 - 1, q7 - 1, q8 - 1, q9 - 1);
}

inline double coupl_6j_base(const int q1, const int q2, const int q3, const int q4, const int q5, const int q6)
{
    return wig6jj(q1 - 1, q2 - 1, q3 - 1, q4 - 1, q5 - 1, q6 - 1);
}
...
coupl_9j_base(const int q1, const int q2, const int q3, const int q4, const int q5, const int q6, const int q7, const int q8, const int q9)
{
    return fw9jja(q1 - 1, q2 - 1, q3 - 1, q4 - 1, q5 - 1, q6 - 1, q7 - 1, q8 - 1, q9 - 1);
}

inline double coupl_6j_base(const int q1, const int q2, const int q3, const int q4, const int q5, const int q6)
{
    return fw6jja(q1 - 1, q2 - 1, q3 - 1, q4 - 1, q5 - 1, q6 - 1);
}
...
{
    return coupl_9j_base(q1, q2, q3, q4, q5, q6, q7, q8, q9);
}

inline double coupling_6j(const int q1, const int q2, const int q3, const int q4, const int q5, const int q6)
{
    return coupl_6j_base(q1, q2, q3, q4, q5, q6);
}

inline double coupling_3j(const int q1, const int q2, const int q3, const int q1_z, const int q2_z, const int q3_z)
{
    return coupl_3j_base(q1, q2, q3, q1_z, q2_z, q3_z);
}
